...
const clockHandsMover = () => {
	...
	second
		? (minuteHand.style.transform =
				leadText + 6 * (minute + second / 60) + minuteText)
		: (minuteHand.style.transform = leadText + 6 * minute + minuteText);
	minute
		? (hourHand.style.transform =
				leadText + 30 * (hour + minute / 60) + hourText)
		: (hourHand.style.transform = leadText + 30 * hour + hourText);
};
